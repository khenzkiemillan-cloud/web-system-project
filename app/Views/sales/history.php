        ...
        <tbody>
            <?php foreach($sales as $sale): ?>
                <tr class="border-b hover:bg-slate-50 transition">
                    <td class="py-3 px-4 font-semibold text-slate-800"><?= $sale['invoice_no'] ?></td>
                    <td class="py-3 px-4 text-slate-500"><?= date('Y-m-d H:i', strtotime($sale['created_at'])) ?></td>
                    <td class="py-3 px-4 font-bold"><?= $sale['customer_name'] ?: 'Guest Walk-in' ?></td>
                    <td class="py-3 px-4 text-right font-extrabold text-indigo-600">₱<?= number_format($sale['total_amount'], 2) ?></td>
                    <td class="py-3 px-4 text-right text-slate-500">₱<?= number_format($sale['amount_paid'], 2) ?></td>
