<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <h2 class="text-lg font-bold mb-4 border-b pb-4 text-slate-800"><i class="fa-solid fa-plus mr-2 text-indigo-600"></i>Stock-In / Adjust Stocks</h2>
            ...
            <div>
                ...
                <select name="product_id" class="w-full border p-3 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none bg-white" required>
                    ...
                    <?php foreach($products as $product): ?>
                        <option value="<?= $product['id'] ?>"><?= $product['name'] ?> (SKU: <?= $product['sku'] ?> | Stock: <?= $product['stock'] ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
    ...
    <div class="lg:col-span-2 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm overflow-x-auto">
        <h2 class="text-lg font-bold mb-4 border-b pb-4 text-slate-800"><i class="fa-solid fa-history mr-2 text-emerald-600"></i>Recent Stock Logs</h2>
            ...
            <tbody>
                <?php foreach($logs as $log): ?>
                    <tr class="border-b">
                        ...
                        <td class="py-3 font-semibold"><?= $log['product_name'] ?> <span class="block text-xs text-slate-400 font-normal">SKU: <?= $log['sku'] ?></span></td>
                        <td class="py-3 text-center"><span class="font-bold text-emerald-600">+<?= $log['change_qty'] ?></span></td>
